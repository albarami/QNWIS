"""
Step 32 Disaster Recovery slice runner.

Runs linting, typing, and DR-specific unit/integration tests with an optional
fast-path check so unrelated changes can skip this heavier slice.
"""
...
LINT_TARGETS = [
    ...
    "src/qnwis/scripts/qa/step32_dr_slice.py",
]
MYPY_TARGETS = [
    "src/qnwis/dr",
    "src/qnwis/scripts/qa/rg7_recovery_gate.py",
...
FAST_PATH_PREFIXES = tuple(
    ...
    for prefix in (
        ...
        "docs/audit/rg7",
        "RUN_RG7_GATE.md",
        "STEP32_DR_IMPLEMENTATION_COMPLETE.md",
        "tests/unit/dr",
        "tests/integration/dr",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Step 32 DR pre-commit slice.")
    parser.add_argument(
        "--files",
        nargs="*",
...
def run_step(name: str, command: list[str]) -> None:
    print(f"[step32] {name}: {' '.join(command)}", flush=True)
...
def main() -> int:
    ...
    if args.files is not None and not should_run(files):
        print("[step32] Fast-path: no DR/RG-7 files changed; skipping slice.")
        return 0
    ...
    print("[step32] Slice completed successfully.")
    return 0
