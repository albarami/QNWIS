...
def fail(msg: str) -> None:
    ...
    print(f"[doc-check] ERROR: {msg}", file=sys.stderr)
    sys.exit(1)


def warn(msg: str) -> None:
    """Print warning."""
    print(f"[doc-check] WARN: {msg}")


def success(msg: str) -> None:
    """Print success message."""
    print(f"[doc-check] OK: {msg}")
...
def check_headings(root: Path) -> None:
    ...
    for doc_path in REQUIRED_DOCS:
        ...
        try:
            ...
            warn(f"Could not read {full_path}: {exc}")

    if missing_headings:
        fail("Documentation files missing H1 heading:\n  " + "\n  ".join(missing_headings))
...
def check_links(root: Path) -> None:
    ...
    for md_file in root.glob("docs/**/*.md"):
        try:
            ...
            warn(f"Could not check links in {md_file}: {exc}")
...
def main() -> None:
    ...
    print(f"[doc-check] Validating documentation in: {root}\n")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[doc-check] Interrupted by user", file=sys.stderr)
        sys.exit(130)
    except Exception as exc:
        print(f"\n[doc-check] Unexpected error: {exc}", file=sys.stderr)
        traceback.print_exc()
        sys.exit(1)
